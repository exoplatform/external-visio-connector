/*
 * Copyright (C) 2024 eXo Platform SAS.
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program. If not, see <http://www.gnu.org/licenses/>.
 */
import ExternalVisio from './components/ExternalVisio.vue';
import ExternalVisioToolbar from './components/ExternalVisioToolbar.vue';
import ExternalVisioDrawer from './components/ExternalVisioDrawer.vue';


const components = {
  'external-visio': ExternalVisio,
  'external-visio-toolbar': ExternalVisioToolbar,
  'external-visio-drawer': ExternalVisioDrawer
};

for (const key in components) {
  Vue.component(key, components[key]);
}

import * as externalVisioConnectorService from './ExternalVisioConnectorService.js';

if (!Vue.prototype.$externalVisioConnectorService) {
  window.Object.defineProperty(Vue.prototype, '$externalVisioConnectorService', {
    value: externalVisioConnectorService,
  });
}